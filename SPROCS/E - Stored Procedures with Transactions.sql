...
-- 1. Add a stored procedure called TransferCourse that accepts a student ID, semester, and two
--    course IDs: the one to move the student out of and the one to move the student in to.
...
-- 3. Create a stored procedure called RegisterStudent that accepts StudentID, CourseID and Semester
--    as parameters. If the number of students in that course and semester are not greater than the
--    Max Students for that course, add a record to the Registration table and add the cost of the
--    course to the students balance. If the registration would cause the course in that semester to
--    have greater than MaxStudents for that course raise an error.
...
-- 4. Add a stored procedure called WitnessProtection that erases all existence of a student from
--    the database. The stored procedure takes the StudentID, first and last names, gender, and 
--    birthdate as parameters. Ensure that the student exists in the database before removing them
--    (all the parameter values must match).
...
-- 5. Create a procedure called StudentPayment that accepts Student ID and paymentamount as parameters.
...
-- 6. Create a stored procedure called WithdrawStudent that accepts a StudentID, CourseId, and
--    semester as parameters. Withdraw the student by updating their Withdrawn value to 'Y' and
--    subtract 1/2 of the cost of the course from their balance. If the result would be a 
--    negative balance set it to 0.
...
-- 7. Create a stored procedure called ArchiveStudentGrades that will accept a year and will archive
--    all student records from that year from the registration table to an ArchiveGrade table. Copy all the
--    appropriate records from the registration table to the ArchiveGrade table and delete them from the
--    registration table. The ArchiveGrade table must have the same column definitions as the registration table
--    but without any check, default, primary or foreign key constraints.
