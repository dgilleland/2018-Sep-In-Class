    ...
    public class SalesController
    {
                ...
                var results = from data in context.Orders.ToList()
                              ...
                              select new CustomerOrder
                              {
                                  ...
                                  Shipper = data.ShipVia.HasValue
                                          ...
                                          : null,
                                  OrderTotal = data.Order_Details.Sum(x => x.Quantity * x.UnitPrice)
                              };
                return results.ToList();
            }
        }
        ...
        public CustomerOrderWithDetails GetExistingOrder(int orderId)
        {
            using (var context = new NorthwindContext())
            {
                ...
                result.Details = (from data in context.Orders
                                  where data.OrderID == orderId
                                  from item in data.Order_Details
        ...
        private void UpdatePendingOrder(EditCustomerOrder order)
        {
            using (var context = new NorthwindContext())
            {
                ...
                foreach (var detail in orderInProcess.Order_Details)
                {
        ...
        public void PlaceOrder(EditCustomerOrder order)
        {
            ...
            using (var context = new NorthwindContext())
            {
                ...
                foreach (var item in order.OrderItems)
                {
                    if (!orderInProcess.Order_Details.Any(x => x.ProductID == item.ProductId))
                    {
                        // Add as a new item
                        var newItem = new Order_Detail
                        {
